...
### TASKS:
1. SPELLING & CLIP OPTIMIZATION: Fix typos. If the query is long, conversational, or complex, paraphrase and rephrase it into a concise, highly descriptive caption optimized for CLIP or similar vision-language models. Focus strictly on visual elements, remove filler words, and keep the core meaning and intent intact (ideally under 30 words). 
...
2. BIAS CHECK: Identify and block any discriminatory or profiling language based on the policy above.
3. SUGGESTIONS: For SAFE queries, provide 3 logical next steps (e.g., "Red car", "Unattended bag", "Person wearing a red jacket"). For UNSAFE queries, suggest 3 generic safety-compliant searches (e.g., "Person carrying a backpack", "Blue vehicle").
